...
<head>
<meta charset="UTF-8">
<title>Day61-EX02.jsp</title>
</head>
<body>
	<%
		Person hong = new Person("홍길동", 25);
		Person lee = new Person("이수진", 26);
	...
	<ul>
		<li>${hong.getName() }</li>
		<li>${kim }</li>
		<%--EL은 null을 출력하지 않는다. 따라서 Attribute가 없어도 널 포인터 예외 발생하지 않음 --%>

		<li>${lee.name }</li>
		<%--EL은 필드명을 작성하면 자동으로getter를 호출하게 되어 있음 --%>

		<li>${list[0].name }</li>
		<li>${map['p2'].name }</li>
		<%--EL은 List, Map접근 시 배열처럼 []을 제공한다. --%>
		
		<li>${hong.name }</li>
		<%--EL은 pageContext부터 순서대로 탐색. 즉, 이름이 겹치면 앞의 영역이 꺼내짐 --%>
