...
select name, round((kor+eng+mat)/3,2) as avg from student;

select sum(kor+eng+mat) as sum,
        avg(kor+eng+mat)/3 as avg
        from student;
...
--view를 조회하는 것은 지정한 select 구문을 매번 실행하는 것이다
-- 즉, 합계와 평균은 조회시마다 매번 새로 구해서 열이 생성된다.
