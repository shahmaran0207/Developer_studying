...
-- count(): 지정한 열의 행의 개수를 반환 → 많이 사용되니 외워두는 것이 좋음
...
-- *는 모든 열을 의미하는데 단순히 테이블 전체 행 개수를 반환할 때 사용함
...
select dep_name,count(*) 
        from employee group by dep_name 
        having count(*) >=2;
...
-- ※select의 실제 완전 구문 (1. select 2. from, 3. position, count(*), 4. where, 5. group by , 6. having 7. order by)
-- ※ where는 그룹화 전
-- ※ having은 그룹화 후
